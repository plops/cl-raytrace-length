  ...
  (let ((start 0)
    ...
    (loop for (curv ind thick) in sys and i from 0 collect
	 (prog1
	   ...
	   (list curv ind (if (= i 0) 
			      start
			     (incf start thick-old)))
  ...
  ;; curv index thick
   `(,(append '(0d0 0d0 0d0)
     ...
     ,(mapcar #'(lambda (x) (+ 1d0 (/ x 1000d0))) 
	      '(525 518 517 0 758 0 762 667 0 667 762 0 489 813 0 813 0 0 0))
     (.17 .281 2.770 .2 2.2 7.127 3. 4. .2 4. 3. 4.823 6.5 4. 1.8 4. .5 100 1))))
			   ...
			   do
				...
				(cl-who:htm (:line :x1 20 :y1 200 ;; optical axis
			       ...
			       (loop for i from 1 below (length *bla2*) and e in *bla2* do
				    (let ((x1 (format nil "~a" (+ 20 (* sc (- 48.571 (aref e 2))))))
					  (y1 (format nil "~a" (+ 200 (* sc (aref e 1)))))
					  (x2 (format nil "~a" (+ 20 (* sc (- 48.571 (aref (elt *bla2* i) 2))))))
